...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  public takeDamage(amount: number): void {
    ...
    const defenseBoost = Math.max(0.1, Math.min((this.scene.registry.get('defenseBoost') as number) || 1, 2));
  ...
  public getAttackDamage(): number {
    const boost = Math.max(0.5, Math.min((this.scene.registry.get('attackBoost') as number) || 1, 3));
